    ...
    <section className="w-full py-14" id="blog-section" style={{ backgroundImage: `url(${bgImg})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col gap-4">
            {supporting.map((blog) => (
              <div key={blog.id} className="group flex gap-4  border border-[#D4A64A]/25 p-3 hover:shadow-sm transition-shadow">
                <div className="w-24 h-24 shrink-0 overflow-hidden bg-[#FAF6F0]">
        ...
        <div className="text-center">
          <Link
            to="/blog"
            className="inline-flex items-center gap-2 border border-[#2D0B0C] text-[#2D0B0C] text-[11px] font-semibold tracking-widest uppercase px-8 py-3 hover:bg-[#2D0B0C] hover:text-white transition-all"
          >
            Read All Blogs <ArrowRight className="w-3.5 h-3.5" />
          </Link>
        </div>
      </div>
    </section>
